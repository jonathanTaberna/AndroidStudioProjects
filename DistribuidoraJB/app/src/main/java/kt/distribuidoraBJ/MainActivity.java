...
import kt.distribuidoraBJ.fragments_datos_cargados.ArticulosCargadosFragment;
import kt.distribuidoraBJ.fragments_datos_cargados.ClientesCargadosFragment;
import kt.distribuidoraBJ.fragments_datos_cargados.ClientesNuevosCargadosFragment;
import kt.distribuidoraBJ.fragments_datos_cargados.PedidosCargadosFragment;
import kt.distribuidoraBJ.fragments_secundarios.ConfiguracionesDialogo;
import kt.distribuidoraBJ.fragments_secundarios.NuevoPedidoFragment;
import kt.distribuidoraBJ.menu_lateral.CambiarPassFragment;
import kt.distribuidoraBJ.menu_lateral.ImportarDatosFragment;
import kt.distribuidoraBJ.menu_lateral.MenuFragment;
import kt.distribuidoraBJ.menu_lateral.NuevoClienteFragment;
    ...
    public void onBackPressed() {
        ...
        if (drawer.isDrawerOpen(GravityCompat.START)) {
            ...
            if (fragmentActual == "importarDatos") {
                if (flagClientesCargados == false && flagArticulosCargados == false && FLAG_IMPORTA_DATOS == 1) {
                    fragmentActual = "menuPrincipal";
                    navigationView.getMenu().getItem(0).setChecked(true); //marca el primer item de la lista del menu lateral
                    llamarNewInstanceMenu(fragmentActual);
                    setearToolbar(false,false,false,false,true, true);
                }
                if (flagClientesCargados == false && flagArticulosCargados == false && FLAG_IMPORTA_DATOS == 0) {
                    drawer.openDrawer(GravityCompat.START);
                }
                if (flagClientesCargados == true || flagArticulosCargados == true) {
